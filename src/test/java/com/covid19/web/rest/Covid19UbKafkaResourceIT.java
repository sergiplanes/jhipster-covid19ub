...
import com.covid19.config.KafkaProperties;
...
class Covid19UbKafkaResourceIT {
    ...
    void setup() {
        ...
        Covid19UbKafkaResource kafkaResource = new Covid19UbKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/covid-19-ub-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/covid-19-ub-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
